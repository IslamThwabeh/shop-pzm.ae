...
var Database = class {
  ...
  async updateProduct(id, updates) {
    try {
      ...
      const updated = { ...product, ...updates, updated_at: (/* @__PURE__ */ new Date()).toISOString() };
      await this.db.prepare(
  ...
  async getOrder(id) {
    try {
      ...
      console.error("Error fetching order:", error3);
      return null;
    }
  }
  ...
  async updateOrder(id, updates) {
    try {
      ...
      const updated = { ...order, ...updates, updated_at: (/* @__PURE__ */ new Date()).toISOString() };
      await this.db.prepare(
      ...
      console.error("Error updating order:", error3);
      return null;
    }
  }
  // ============ ADMIN ============
  async getAdminByUsername(username) {
    try {
      const result = await this.db.prepare("SELECT id, username, email, role, created_at FROM admin_users WHERE username = ?").bind(username).first();
      return result || null;
    } catch (error3) {
      console.error("Error fetching admin:", error3);
      return null;
    }
  }
  async getAdminPasswordHash(username) {
    try {
      const result = await this.db.prepare("SELECT password_hash FROM admin_users WHERE username = ?").bind(username).first();
      return result?.password_hash || null;
    } catch (error3) {
      console.error("Error fetching admin password:", error3);
      return null;
    }
  }
};
...
var AuthService = class {
  ...
  async verifyToken(token) {
    try {
      ...
      console.error("Error verifying token:", error3);
      return null;
    }
  }
  /**
   ...
   */
  async verifyPasswordHash(receivedHash, storedHash) {
...
var EmailService = class {
  ...
  apiToken;
  senderEmail = "support@pzm.ae";
  teamEmail = "support@pzm.ae";
  ...
  async sendEmail(options) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error("ZeptoMail API Error:", error3);
        return false;
      }
   ...
   */
  async sendOrderConfirmation(customerEmail, customerName, orderId, productModel, quantity, totalPrice) {
   ...
   */
  async sendOrderNotification(orderId, customerName, customerEmail, customerPhone, productModel, quantity, totalPrice) {
   ...
   */
  async sendStatusUpdate(customerEmail, customerName, orderId, status, productModel) {
   ...
   */
  getOrderConfirmationTemplate(customerName, orderId, productModel, quantity, totalPrice) {
    return `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div class="container">
          ...
          <div class="footer">
            <p>\xA9 2025 PZM iPhone Store. All rights reserved.</p>
   ...
   */
  getOrderNotificationTemplate(orderId, customerName, customerEmail, customerPhone, productModel, quantity, totalPrice) {
    return `
      <!DOCTYPE html>
      <html>
      <head>
   ...
   */
  getStatusUpdateTemplate(customerName, orderId, status, productModel) {
    const statusInfo = {
      confirmed: {
        emoji: "\u2713",
    ...
    const info3 = statusInfo[status] || statusInfo.confirmed;
    return `
      <!DOCTYPE html>
      <html>
      <head>
        ...
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { background: linear-gradient(135deg, ${info3.color} 0%, ${info3.color}dd 100%); color: white; padding: 30px; text-align: center; border-radius: 8px 8px 0 0; }
          .header h1 { margin: 0; font-size: 28px; }
          .header .emoji { font-size: 40px; margin-bottom: 10px; }
          .content { background: #f9f9f9; padding: 30px; border: 1px solid #ddd; border-radius: 0 0 8px 8px; }
          .status-message { background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid ${info3.color}; }
          .order-details { background: white; padding: 20px; border-radius: 8px; margin: 20px 0; }
          .detail-row { display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #eee; }
          .detail-row:last-child { border-bottom: none; }
          .label { font-weight: bold; color: ${info3.color}; }
          .footer { text-align: center; padding: 20px; color: #666; font-size: 12px; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="header">
            <div class="emoji">${info3.emoji}</div>
            <h1>${info3.title}</h1>
          </div>
          ...
          <div class="content">
            ...
            <div class="status-message">
              <p><strong>${info3.message}</strong></p>
            </div>
...
function validateRequired2(obj, fields) {
  return validateRequired(obj, fields);
}
__name(validateRequired2, "validateRequired");
async function parseRequestBody(c) {
  try {
    ...
    console.error("Error parsing request body:", error3);
    return null;
  }
}
...
function logRequest(method, path, data) {
  const sanitized = data ? sanitizeForLogging(data) : void 0;
  console.log(`[${(/* @__PURE__ */ new Date()).toISOString()}] ${method} ${path}`, sanitized || "");
}
__name(logRequest, "logRequest");
function logError(error3, context2) {
  const timestamp = (/* @__PURE__ */ new Date()).toISOString();
  const message = error3 instanceof Error ? error3.message : String(error3);
  console.error(`[${timestamp}] ERROR${context2 ? ` (${context2})` : ""}: ${message}`);
}
...
var PLACE_ID = "ChIJ1aZJvMBtXz4RLrOI1vITjBU";
app.get("/api/business-hours", async (c) => {
  try {
    ...
    logError(error3, "GET /api/business-hours");
...
app.get("/api/products", async (c) => {
  try {
    ...
    const products = await db.getProducts(condition);
    return c.json({ data: products, status: 200 }, 200);
  } catch (error3) {
    logError(error3, "GET /api/products");
...
app.get("/api/products/:id", async (c) => {
  try {
    ...
    logError(error3, "GET /api/products/:id");
...
app.post("/api/products", async (c) => {
  try {
    ...
    const validation = validateRequired2(body, ["model", "storage", "condition", "color", "price"]);
    if (validation) {
    ...
    logError(error3, "POST /api/products");
...
app.put("/api/products/:id", async (c) => {
  try {
    ...
    logError(error3, "PUT /api/products/:id");
...
app.delete("/api/products/:id", async (c) => {
  try {
    ...
    logError(error3, "DELETE /api/products/:id");
...
app.get("/api/orders", async (c) => {
  try {
    ...
    logError(error3, "GET /api/orders");
...
app.get("/api/orders/:id", async (c) => {
  try {
    ...
    logError(error3, "GET /api/orders/:id");
...
app.post("/api/orders", async (c) => {
  try {
    ...
    const validation = validateRequired2(body, [
      "customer_name",
      "customer_email",
      "customer_phone",
      "customer_address",
    ...
    logError(error3, "POST /api/orders");
...
app.put("/api/orders/:id", async (c) => {
  try {
    ...
    logError(error3, "PUT /api/orders/:id");
...
app.post("/api/auth/admin/login", async (c) => {
  try {
    ...
    const validation = validateRequired2(body, ["username", "password"]);
    if (validation) {
      return c.json({ error: validation, status: 400 }, 400);
    }
    const db = new Database(c.env.DB);
    ...
    logError(error3, "POST /api/auth/admin/login");
...
app.get("/api/auth/verify", async (c) => {
  try {
    ...
    logError(error3, "GET /api/auth/verify");
...
// .wrangler/tmp/bundle-YljzWB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-YljzWB/middleware-loader.entry.ts
